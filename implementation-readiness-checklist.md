...
- [ ] CI-Hard-Fail für unbekannte direkte oder transitive `Avalonia*`-/AvaloniaUI-Pakete, Premium-Paketmarker und Avalonia-Lizenzschlüsselmarker anlegen. Teilweise durch `FND-006`: der [Secret-Marker-Scanner](eng/check-secret-markers.ps1) samt Negativkontrolle ist als Hard-Gate in [`.github/workflows/ci.yml`](.github/workflows/ci.yml) verdrahtet und deckt die Lizenzschlüsselmarker (`AvaloniaUILicenseKey`, `AVALONIA_TOOLS_LICENSE_KEY`, `ACCELERATE_LICENSE_KEY`) ab. Das vollständige Gate für unbekannte `Avalonia*`-Pakete, Premium-Paketmarker und Portalabhängigkeiten folgt in `FND-009`; deshalb bleibt der Punkt offen.
...
- CI-Lauf: Grundgerüst mit `FND-006` umgesetzt: [`.github/workflows/ci.yml`](.github/workflows/ci.yml) führt auf sauberem Checkout Locked-Restore, Release-Build mit `TreatWarningsAsErrors`, Tests (inklusive Architekturtests) sowie den [FluentAssertions-Scanner](eng/check-fluent-assertions.ps1) und den [Secret-Marker-Scanner](eng/check-secret-markers.ps1) je mit Negativkontrolle als Hard-Gates aus. Noch offen für dieses Gate: Formatierungsprüfung, Headless-VisualTests (`UI-007`), vollständiges Lizenz-/Avalonia-Free-/Dependency-Gate samt `THIRD-PARTY-NOTICES.txt`-Prüfung (`FND-009`), Native-AOT-Publish je Ziel-OS und Screenshot-/Log-Artefakte.
